...
- **Network privacy.** There is no Tor/proxy support. Your Electrum server
  ...
  automatically fetches spot price (~60s while unlocked), fee hints (send
  form), or Insights pulse/charts (~90s / on view). It learns IP + a specific
  txid when you open a transaction detail (enrichment) or click “View on
  litview” (system browser). Fiat, fee-hint, and Insights fetches can be
  disabled in Settings. Wallet address lists are never uploaded to the
  explorer. See [`docs/LITVIEW.md`](docs/LITVIEW.md).
